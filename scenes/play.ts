...
export class Play extends Phaser.Scene {
  ...
  create() {
    ...
    if(!this.game.renderer.hasPipeline("distortion")) {
      //@ts-ignore 
    ...
    // Instantiate planets.
    for(let i = 0; i < 8; i++) {
      let p = new CelestialBody(this,  80 + (i*32), GAME_HEIGHT / 2, i);
      this.planets.push(p);
    } 

    // Instantiate moon.
    this.moon = new Moon(this, this.planets[2].x, this.planets[2].y - CB_SIZE - 1);
  ...
  update() {
    ...
    for(let i = 0; i < this.planets.length; i++) {
      ...
      var rThres = this.planets[i].isSlide ? .4 : 0;

      rThres = this.planets[i].isGravity || 
               this.planets[i].isRepulsion || 
               this.planets[i].isAttraction ? 1.9 : rThres;
      ...
      if(!this.bChangedPos && this.moonCooldown && this.moonCanShoot) {
          ...
          if(this.planets.length > 0) {
            ...
            this.time.delayedCall(2000, function(){
              if(this.moon == null)  {
              this.moon = new Moon(this, this.blackHole.x, this.blackHole.y);
