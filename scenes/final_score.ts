...
export class FinalScore extends Phaser.Scene {
   ...
   canGoMenu : boolean; 
    ...
    create() {
      ...
      this.score = 8000 - this.elapsedTime - (this.nShoot * 100) - (this.nMoonBlack * 100);

      var hs = localStorage.getItem("classic_hiscore");

      // Set the highest score
      if(hs == null || Number(hs) < this.score)
        localStorage.setItem("classic_hiscore", String(this.score));
      ...
      "\n\nSCORE CALCULATION:\n8000 - " + String(this.elapsedTime) + " - (" + String(this.nShoot) + " x 100) - (" 
